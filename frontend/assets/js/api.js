/**
 * EduPulse - Shared frontend API client.
...
const EduAPI = (() => {
  ...
  const API_BASE = window.EDUPULSE_API_BASE || (location.protocol === 'file:' ? 'http://localhost:4000' : location.origin);

  if (location.protocol === 'file:') {
    console.warn('[EduPulse] Trang đang mở bằng file:// — hãy chạy backend (npm start) rồi mở http://localhost:4000 thay vì mở trực tiếp file HTML.');
  }
  ...
  return { API_BASE, getToken, setToken, clearToken, getUser, setUser, request, requireLogin, openChatSocket, fileUrl, toast, enablePasswordToggle };
})();
