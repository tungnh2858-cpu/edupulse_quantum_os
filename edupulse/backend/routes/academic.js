...
router.post('/schools', requireAuth, uploadLogo.single('logo'), (req, res) => {
...
router.put('/schools/:id', requireAuth, uploadLogo.single('logo'), (req, res) => {
  const db = getDB();
...
router.delete('/schools/:id', requireAuth, requireAdmin, (req, res) => {
  const db = getDB();
...
router.put('/classes/:id/students', requireAuth, (req, res) => {
  if (!['admin', 'teacher'].includes(req.user.role)) return res.status(403).json({ error: 'Không đủ quyền.' });
