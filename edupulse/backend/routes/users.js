...
// GET /api/users/directory (any authed user: minimal list for chat/social @mentions)
...
// POST /api/users  (admin creates account: student / teacher / admin)
router.post('/', requireAuth, requireAdmin, (req, res) => {
  const { username, password, role, fullName, email } = req.body || {};
  if (!username || !password || !role) return res.status(400).json({ error: 'Thiếu thông tin bắt buộc.' });
  if (!['admin', 'teacher', 'student'].includes(role)) return res.status(400).json({ error: 'Vai trò không hợp lệ.' });
...
// PUT /api/users/:id/role  (admin upgrades/downgrades a role)
...
// PUT /api/users/:id/status (lock / unlock account)
...
// ---- Self-service settings (any logged-in user) ----
