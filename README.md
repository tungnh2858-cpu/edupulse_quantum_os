...
Nếu muốn tách frontend ra host riêng (ví dụ Nginx/Vercel/Netlify), mở `frontend/assets/js/api.js` và đặt:

```js
window.EDUPULSE_API_BASE = 'https://your-backend-domain.com';
```

trước khi các file JS khác được nạp (thêm 1 dòng `<script>` nhỏ ở đầu mỗi trang HTML).
